...
BEGIN
    ...
    /* CTE to Define the 4 fixed stages */
    with stages AS (
    ...
    SELECT 
        ...
        st.Stage,
        coalesce(s.Status, 'Fail') AS Status, /* If a student doesn't reached a stage that means he/she 'Failed' */
        ...
        (SELECT concat(ROUND(sum(Status='Pass') * 100.0 / count(*), 1), ' %')
         FROM time_travel as t /* name of table containing exam data of students */
        ...
        /* Peer stats by Age */
        (SELECT concat(ROUND(sum(Status='Pass') * 100.0 / count(*), 1), ' %')
         FROM time_travel t /* name of table containing exam data of students */
         WHERE t.Stage = st.Stage
